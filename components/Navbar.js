...
const Navbar = () => {
    ...
    return (
        <nav className="bg-white font-mono shadow-md sticky top-0 z-50 dark:bg-neutral-800 transition duration-500 dark:border-b-2 dark:border-zinc-800">
            ...
            <div className={`${isOpen ? "flex" : "hidden"} justify-center items-center bg-gray-300 dark:bg-neutral-800 transition duration-500 dark:border-t-2 dark:border-black h-[calc(100vh_-_theme(space.24))]`}>
                <ul className="w-screen md:hidden">
                    {
                        MenuItems.map((navigate, id) => (
                            <li key={id} className="text-center hover:bg-gray-600 block text-lg px-2 py-4 text-black dark:text-white" onClick={closeMobileMenu}>
                                <Link href={navigate.link}>
                                    {navigate.name}
                                </Link>
                            </li>
                    ...
                    <span onClick={() => setTheme(colorTheme)} className="cursor-pointer mx-auto justify-center flex">
                        {colorTheme === "dark" ?
                            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
                            </svg> :
